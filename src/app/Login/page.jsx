  ...
  const handlereset = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${API_URL}request_password_reset?uhid=${resetUHID}&role=doctor&email=${resetEmail}`
      );
    ...
    <div className="relative bg-[#CFDADE] min-h-screen w-full">
      ...
      <div className="absolute inset-0 p-2 box-border rounded-4xl">
        <div className="w-full h-full rounded-4xl border-white border-[1px] bg-white/10 ring-1 ring-white/30 backdrop-blur-sm p-1 shadow-[0_0_0_0.5px_rgba(255,255,255,0.3)]">
          <div
            className={`w-full h-full rounded-4xl bg-white/20 backdrop-blur-none text-white flex flex-row justify-center `}
          >
            ...
            <div
              className={`${
                width >= 1100
                  ...
                  ? "w-3/5"
                  : "w-full px-8"
              } flex flex-col h-full justify-center items-center py-4`}
            >
              <div className="flex flex-col h-full w-full justify-center items-center bg-white rounded-[15px]">
                <div className={`w-full h-1/2 p-2`}>
                  <Image
                    src={Doctor}
                    alt="Doctor Cover Image"
                ...
                {!showForgotPassword && (
                  <div
                    className={`w-full h-1/2 flex flex-col justify-start items-start gap-4 px-8`}
                  >
                    {/* Form */}
                    <h2
                      className={`${inter.className} text-[25px] font-bold text-black w-fit`}
                    >
                      Sign in
                    </h2>
                    ...
                    <form
                      ...
                      } ${height < 770 ? "py-4 h-full" : " h-full py-8"}`}
                      onSubmit={(e) => {
                    ...
                    >
                      ...
                      <div className="w-full flex items-center justify-center">
                        <button
                          type="submit"
                          disabled={loginlock}
                          className={`${raleway.className} w-4/5 text-lg cursor-pointer bg-black text-white rounded-md py-1 font-semibold hover:bg-gray-800 transition`}
                        >
                          {loginlock ? (
                            // Spinner + text
